	...
	<div style="margin-left:25%;padding:1px 16px;height:1000px;">
		<p style="margin-left: 10%; margin-top: 5%; font-size: 28px;"></p>
			<table class="basic_box">
				...
				<tr>
				<?php
$phone = $row[0] ?? null; // Ensure $row[0] exists
$id = $row[4] ?? null;     // Ensure $row[4] exists

$sql1 = "SELECT * FROM booked_hist";
if ($result = mysqli_query($conn, $sql1)) {
    while ($row = mysqli_fetch_row($result)) {
        // Ensure the indexes exist before accessing them
        if (isset($row[0], $row[2]) && $row[0] == $phone && $row[2] == $id) {
            ?>
            <tr>
                <td><?php echo $row[14] ?? 'N/A'; ?></td>
                <td><?php echo $row[1] ?? 'N/A'; ?></td>
                <td><?php echo $row[3] ?? 'N/A'; ?></td>
                <td><?php echo $row[4] ?? 'N/A'; ?></td>
                <td><?php echo $row[5] ?? 'N/A'; ?></td>
                <td><?php echo $row[13] ?? 'N/A'; ?></td>
            </tr>
            <?php
        }
    }
